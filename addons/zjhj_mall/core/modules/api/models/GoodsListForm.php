...
class GoodsListForm extends Model
{
    ...
    public function search()
    {
        ...
        $list = $query
            ...
            ->select('g.id,g.name,g.price,g.service,g.original_price,g.cover_pic pic_url,gn.num,g.virtual_sales,g.unit')
...
*/
        $query=Goods::find()->alias('g')
                            ->where(['and',"g.id!=$goods_id","store_id=$this->store_id",'is_delete=0','status=1',['in','cat_id',$goods_cat]]);
        $count = $query->count();
        $pagination = new Pagination(['totalCount' => $count, 'pageSize' => $this->limit, 'page' => $this->page - 1]);
...
//        ,'name','price','original_price','pic_url','num','virtual_sales','unit'
        ...
        $list = $query
            ->select(['id','name','cover_pic as pic_url','service','price','original_price','virtual_sales as sales','unit'])
            ->limit($pagination->limit)
            ->offset($pagination->offset)
            ->asArray()->all();
        return [
