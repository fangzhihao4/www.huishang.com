...
class Cloud
{
	...
	public static function api2Get($url, $data = array())
	{
		$site_info = self::getSiteInfo();
		$data = array_merge($data, ['_domain' => $site_info['host'], '_version' => $site_info['version'], '_product' => self::$product]);
		$curl = self::getCurl();
		$curl->get($url, $data);
		return $curl;
		
	}
	public static function api2Post($url, $data = array())
	{
		$site_info = self::getSiteInfo();
		$query_str = http_build_query(['_domain' => $site_info['host'], '_version' => $site_info['version'], '_product' => self::$product]);
		$url = $url . '?' . $query_str;
		$curl = self::getCurl();
		$curl->get($url, $data);
		return $curl;
	...
	public static function getAuthInfo()
	{
		...
		if (!$curl->response) {
			...
			self::$auth_info = ['code' => 1, 'msg' => '服务器授权失败，云服务器连接出错'];
		}
	...
	public static function checkAuth()
	{
		if (\Yii::$app->request->isGet && !\Yii::$app->request->isAjax) {
			...
			if ($auth_info['code'] != 0) {
				echo \Yii::$app->view->renderFile('@app/hejiang/views/auth_error.php', ['msg' => $auth_info['msg']]);
				\Yii::$app->end();
			}
		}
	}
