...
class TeamForm extends Model
{
    ...
    public function getOrder()
    {
        ...
        if (!empty($team_arr['f_c'])) {
            $andWhere .= "(al.parent_id_2 = 0";
            $andWhere_1 .= " OR al.parent_id_1 = {$this->user_id} ";
        }else{
            $andWhere .= "(al.parent_id_1 = {$this->user_id}";
        }
        if(!empty($team_arr['s_c'])){
            $andWhere .= " AND ((al.user_id IN {$second} AND al.parent_id_1 IN {$first}) ";
            $andWhere_1 .= " OR al.parent_id_2 = {$this->user_id}";
        }else{
            $andWhere .= " OR (al.parent_id_2 = {$this->user_id}";
        }
        if(!empty($team_arr['t_c'])){
            $andWhere .= " OR (al.user_id IN {$third} AND al.parent_id_1 IN {$second}))) ";
            $andWhere_1 .= " OR al.parent_id_3 = {$this->user_id} ";
        }else{
            $andWhere .= ")) ";
        }
        $andWhere .= $andWhere_1;
        $sql = $select . $sql . " LEFT JOIN {$user_table} AS u ON u.id=al.user_id
        WHERE ((
            al.user_id != {$this->user_id} AND ({$andWhere})
        ) OR (al.user_id = {$this->user_id} AND al.rebate > 0)) ";
        if ($this->status == 0) {
