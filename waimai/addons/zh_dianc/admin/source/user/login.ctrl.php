...
//	}
	...
	if(!empty($record)) {
		...
		$account = pdo_fetch("SELECT * FROM " . tablename("wpdc_account") . " WHERE status=2 AND uid=:uid ORDER BY id DESC LIMIT 1", array(':uid' => $record['uid']));
		if (!empty($account)) {
...
//		message("欢迎回来，{$record['username']}。", $forward);
		...
		pdo_update("wpdc_account", $data, array('id' => $record['id']));

		message("欢迎回来，{$record['username']}！", url('site/entry/stores', array('m' => 'zh_dianc', 'id' => $storeid, 'do' => 'start')), 'success');
	} else {
		if (empty($failed)) {
