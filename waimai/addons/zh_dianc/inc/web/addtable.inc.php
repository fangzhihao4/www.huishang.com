...
function  getCoade($table_id,$storeid){
     ...
     function set_msg($table_id,$storeid){
       $access_token = getaccess_token();
        $data2=array(
				...
				"page"=>"zh_dianc/pages/index/index",
				"width"=>400
               );
  ...
  return $img;
	...
	if(!$_GPC['id']==''){
		...
		$list = pdo_get('wpdc_table',array('uniacid' => $_W['uniacid'],'id'=>$_GPC['id']));
		...
		if(checksubmit('submit')){
			...
			if($_GPC['id']==''){
			$data['status']=0;		
				$res=pdo_insert('wpdc_table',$data);
				if($res){
					message('添加成功',$this->createWebUrl('table2',array()),'success');
				}else{
					message('添加失败','','error');
				}
			}else{		
			$res = pdo_update('wpdc_table', $data, array('id' => $_GPC['id']));
				if($res){
