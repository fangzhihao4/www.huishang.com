...
$stores = pdo_fetchall("SELECT * FROM " . tablename('wpdc_store') . " WHERE uniacid= :weid ORDER BY id DESC", array(':weid' => $_W['uniacid']), 'id');
    ...
    if (!empty($id)) {
        ...
        $account = pdo_fetch("SELECT * FROM " . tablename('wpdc_account') . " WHERE weid = :weid AND id=:id ORDER BY id DESC", array(':weid' => $_W['uniacid'], ':id' => $id));
        ...
        if (empty($users)) {
            ...
            if ($uid > 0) {
                ...
                pdo_insert('users_permission', array(
                    ...
                    'permission' => 'zh_dianc_menu_store'
