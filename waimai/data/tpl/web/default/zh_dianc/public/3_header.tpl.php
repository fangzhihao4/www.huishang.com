...
<style>
	...
	.fa-pencil-square-o,.fa-comments,.fa-star-half-o,.fa-users,.fa-user,.fa-clipboard,.fa-money,.fa-life-ring,.fa-bars,.fa-recycle,.fa-cog,.fa-book,.fa-database,.fa-gift,.fa-user,.fa-binoculars,.fa-trophy,.fa-key{margin-right: 15px;}
	.fa-cubes{margin-right: 8px;}
	.fa-university{margin-right: 10px;}
	.fa-map-marker{margin-right: 18px;}
	.fa-jpy{margin-right: 20px;}
</style>

<?php  $res=pdo_get('wpdc_system',array('uniacid'=>$_W['uniacid']))?>
<div class="header">
	<div class="wheaderbox">
		<img class="header_logo" src="<?php  if($res['link_logo']) { ?><?php  echo tomedia($res['link_logo'])?><?php  } else { ?>../addons/zh_dianc/template/images/bac.png<?php  } ?>" alt=""/>
		<span class="header_title">
			<?php  if($res['link_name']) { ?><?php  echo $res['link_name'];?>
			<?php  } else { ?>智慧餐饮管理系统<?php  } ?>
		</span>
	</div>
    <div class="header_left">        
        <?php  if($_W['role'] == 'founder') { ?>
        <a href="<?php  echo $this->createWebUrl('gaikuangdata')?>"><div class="header_btn">返回首页</div></a>
         <?php  } ?>  
    </div>
    <ul class="header_right">       
        ...
        <li>
            ...
            <!-- <img src="../addons/zh_dianc/template/images/kaiguan.png" width="20px" height="20px" title="退出系统"/> -->
...
box-sizing: border-box;margin-top:10px;padding: 10px;border: 1px solid #f2f2f2;box-sizing: border-box;max-width: 100%;" alt="image" src="<?php  echo tomedia($cur_store['md_logo'])?>" onerror="this.src='../addons/zh_dianc/template/images/logo.png';"/>
						</span>
						<a href="#" >
					...
					<script type="text/javascript">
						/*$(".navtitle").each(function(){
							$(this).click(function(){
								// console.log($(this).children(".therji").attr('id'))
								var thid = $(this).children(".therji").attr('id');
								$(this).children(".therji").show();
								$(this).children(".wytitle").addClass("wyactive");
								var erjinav = $(this).parent().siblings()
								$(this).parent().siblings(".panel").children().children().removeClass("wyactive");
								// console.log($(this).parent().siblings(".panel").children().attr("class"))
								for(var i=0;i<erjinav.length;i++){
									// console.log(erjinav.children().children(".therji").attr('id'))
									erjinav.children().children(".therji").hide();
								}								
