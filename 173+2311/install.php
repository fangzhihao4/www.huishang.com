	...
	if($ispost) {
		...
		if(empty($error)) {
			...
			if($_POST['type'] == 'remote') {
				...
				if($ins == 'error') {
					die('<script type="text/javascript">alert("版本错误，请确认是否为小程序商城最新版安装文件！");history.back();</script>');
				}
			...
			if(file_exists(IA_ROOT . '/index.php') && is_dir(IA_ROOT . '/web') && file_exists($verfile) && file_exists($dbfile)) {
				...
				die('<script type="text/javascript">alert("你正在使用本地安装, 但未下载完整安装包, 请从小程序商城官网下载完整安装包后重试.");history.back();</script>');
			}
...
<html lang="zh-cn">
	<head>
		...
		<title>安装系统 - 小程序商城 - 公众平台自助开源引擎</title>
	...
	echo <<<EOF
		<div class="panel panel-default">
			...
			<div class="panel-body" style="overflow-y:scroll;max-height:400px;line-height:20px;">
				<h3>版权所有 (c)2014，小程序商城保留所有权利。 </h3>
				<p>
					感谢您选择小程序商城 - 微信公众平台自助开源引擎（基于 PHP + MySQL的技术开发，全部源码开放。 <br />
					为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：
				</p>
				<p>
					<strong>一、本授权协议适用且仅适用于小程序商城系统(, MicroEngine. 以下简称小程序商城)任何版本，小程序商城官方对本授权协议的最终解释权。</strong>
				</p>
				<p>
					<strong>二、协议许可的权利 </strong>
					<ol>
						<li>您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用。</li>
						<li>您可以在协议规定的约束和限制范围内修改小程序商城源代码或界面风格以适应您的网站要求。</li>
				...
				<p>
					...
					<ol>
						...
						<li>未经官方许可，禁止在小程序商城的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。</li>
...
function tpl_install_env($ret = array()) {
	...
	echo <<<EOF
		...
		<div class="alert alert-info">系统要求小程序商城整个安装目录必须可写, 才能使用小程序商城所有功能。</div>
		<div class="panel panel-default">
...
function tpl_install_finish() {
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“小程序商城 - 公众平台自助开源引擎”系统，您现在可以：　 <a target="_blank" class="btn btn-success" href="./web/index.php">访问网站首页</a>
	</div>
EOF;
	tpl_frame();
}
